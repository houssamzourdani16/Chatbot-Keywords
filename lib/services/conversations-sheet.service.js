 ...
 *   - Column B (sender_id_column): the Sender ID
 *   - Columns C..Z: each cell holds one message from that sender's history
 *
...
export async function saveConfig(data, id) {
  ...
  const payload = {
    ...
    columns: {
      sender_id_column: data.columns?.sender_id_column ?? 1,
...
export async function getConversationBySender(senderId) {
  ...
  for (const config of activeConfigs) {
    try {
      const { rows } = await readAllRows(config);
      const senderCol = config.columns?.sender_id_column ?? 1;
 ...
 *
 * Finds the sender's row (by sender ID in Column B) and writes the NEW
 ...
 * exist yet, creates a NEW row with the sender ID in Column B and the
 * message in Column C.
 *
...
export async function appendMessageToSheet(senderId, message) {
  ...
  try {
    ...
    const senderCol = targetConfig.columns?.sender_id_column ?? 1;
    ...
    if (matchRowIndex >= 0) {
      ...
      const firstMsgCol = senderCol + 1; // Column C by default (sender in B)
      ...
      const newRowValues = [];
      newRowValues[senderCol] = String(senderId);
      newRowValues[senderCol + 1] = String(message);
      newRowValues.push(...existingMessages);

      // Write the whole row starting at the sender ID column.
      ...
      //    Sender ID in Column B (senderCol), message in Column C.
