...
import { saveUnfoundKeywords } from "@/lib/services/unfound-keyword.service";
...
export async function processBatch(batchId, { force = false } = {}) {
  ...
  try {
    ...
    let allKeywordData = {};
    // ✅ ALL keywords from the product's keyword list (the whole sheet),
    //    not just the ones found in the message.
    let allKeywords = [];
    try {
      ...
      // ✅ Fetch the ENTIRE keyword list (all keywords with full details)
      //    from the product's linked Google Sheet, so n8n gets the whole
      //    sheet — not just the keywords found in this message.
      if (product?.keyword_list_id) {
        try {
          const { keywords = [] } = await getKeywordsForList(
            product.keyword_list_id.toString(),
          );
          allKeywords = keywords;
        } catch (listError) {
          console.error(
            "⚠️ Failed to load full keyword list:",
            listError.message,
          );
        }
      }
      ...
      // ✅ Save UNFOUND keywords to the database so admins can review,
      //    manage, and delete them. Non-fatal: if this fails, the message
      //    still processes.
      if (keywordResults.unfound && keywordResults.unfound.length > 0) {
        try {
          await saveUnfoundKeywords({
            user_id: batch.user_id,
            product_id: batch.product_id,
            sender_id: batch.sender_id,
            message: allText,
            keywords: keywordResults.unfound,
          });
        } catch (unfoundError) {
          console.error(
            "⚠️ Failed to save unfound keywords:",
            unfoundError.message,
          );
        }
      }
    } catch (kwError) {
    ...
    // ============================================
    // ✅ RESOLVE DESTINATION WEBHOOK + LOAD PRODUCT
    //    Priority:
    //    1. product.webhook_model_id → WebhookModel.webhook_url
    //    2. product.webhook_url
    //    3. BATCH_WEBHOOK_URL env var (fallback)
    //    Also loads the FULL product so we can send all its details
    //    (catalog, pricing, stock, etc.) to the AI agent in n8n.
    // ============================================
    let destinationUrl = null;
    let product = null;
    try {
      product = await Product.findById(batch.product_id).lean();
      if (product) {
    ...
    let sendResult = await sendToN8N({
      ...
      extra: {
        ...
        // ✅ The ENTIRE keyword list from the product's Google Sheet
        //    (all keywords with full details, not just the found ones).
        ...
        lead_detected: lead
          ...
          : null,
        // ✅ Full past conversation history from the database
        ...
        // ✅ Full past conversation history WITH TIMESTAMPS (each entry is
        //    { text, created_at }) so the AI in n8n knows WHEN each message
        //    was sent.
        conversation_history_with_timestamps: conversationHistory.history || [],
        // ✅ Messenger access token for this product's page (so the AI
        //    agent in n8n can reply on behalf of the page).
      ...
      if (fallbackUrl && fallbackUrl !== destinationUrl) {
        ...
        sendResult = await sendToN8N({
          ...
          extra: {
            ...
            conversation_history_found: conversationHistory.found,
            conversation_history_sources: conversationHistory.sources,
            conversation_history_with_timestamps:
