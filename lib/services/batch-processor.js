...
export async function processBatch(batchId, { force = false } = {}) {
  ...
  try {
    ...
    // ============================================
    // ✅ CONVERSATION HISTORY (Google Sheets)
    //    Before calling n8n, read the sender's FULL past conversation
    //    history from the linked conversations sheets. This gives the AI
    //    in n8n complete context of everything this sender has said.
    //    Non-fatal: if the sheets aren't configured or fail, we still send.
    // ============================================
    ...
    // ============================================
    // ✅ ARCHIVE MESSAGES TO CONVERSATIONS SHEET
    //    When a message arrives, write it to the linked conversations
    //    sheet: find the sender's row (by sender ID in Column A) and
    //    append the message to the next empty column. If the sender
    //    doesn't exist yet, create a NEW row.
    //    Non-fatal: if the sheet isn't configured or write fails, we
    //    still send to n8n.
    // ============================================
    try {
      for (const msg of conversation) {
        ...
        if (text) {
          await appendMessageToSheet(batch.sender_id, text);
        }
      }
    } catch (archiveError) {
      console.error(
        "⚠️ Archive to conversations sheet error:",
