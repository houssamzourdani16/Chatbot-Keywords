...
export async function sendToN8N({
  ...
  const payload = {
    ...
    keywords: {
      ...
      all_keywords: extra.all_keywords || [],
    },

    // ===== 5. CONVERSATION HISTORY (past messages from the database) =====
    conversation_history: {
      // ✅ Each past message WITH its timestamp: [{ text, created_at }]
      //    so the AI in n8n knows WHEN each message was sent.
      messages: (extra.conversation_history_with_timestamps || []).map((h) => ({
        text: h.text,
      ...
      // ✅ Backwards-compatible plain strings (no timestamps)
      plain_messages: extra.conversation_history || [],
    },
