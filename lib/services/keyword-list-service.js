...
export async function readKeywordsFromSheet(keywordList) {
  ...
  if (!api_key && !keywordList.google_sheets.service_account_email) {
    ...
    const headers = (result.headers || []).map((h) => String(h).trim());

    // ✅ Keep the FULL row (all columns) for each keyword.
    //    Convert the row object (keyed by header) into an array aligned
    //    with `headers`, so downstream detection can label each cell.
    const keywords = result.rows.map((row, i) => {
      const rowArray = headers.map((h) => row[h] || "");
      return {
        keyword:
          result.keywords[i] || rowArray[columns?.keyword_column || 0] || "",
        category: result.categories[result.keywords[i]] || "Other",
        // All columns preserved as an array + the headers for labeling
        data: rowArray,
        headers,
      };
    });
  ...
  try {
    ...
    // ✅ Capture the header row (if present) so we can label each column
    //    when displaying the full row for a matched keyword.
    const headerRow =
      startRow > 0 ? rows[0].map((h) => String(h).trim()) : null;
    ...
    for (let i = startRow; i < rows.length; i++) {
      ...
      // ✅ Keep the FULL row (all columns) so downstream detection can
      //    return every spreadsheet column for a matched keyword.
      //    Also attach the headers so the UI can label each cell.
      keywords.push({
        keyword,
        category,
        data: row,
 ...
 * Also recognizes the actual column names used in the keyword sheets
 * (darija, latin, meaning, category, etc.).
 */
