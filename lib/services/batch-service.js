...
export async function addMessageToBatch({
  ...
  if (!batch) {
    // ✅ If there's no OPEN batch, check if there's a batch still being
    //    PROCESSED for this sender. If so, we REUSE it (reset to open and
    //    extend the timer) so all messages from the same sender stay in
    //    ONE batch and are processed together. This prevents messages from
    //    being split when a new message arrives while the previous batch
    //    is mid-processing.
    //
    //    NOTE: We only reuse "processing" batches — NOT "completed" or
    //    "failed" ones — because those have already been sent to n8n and
    //    reusing them would re-send the old messages.
    const processingBatch = await Batch.findOne({
      product_id,
      sender_id,
      status: "processing",
    }).sort({ created_at: -1 });
