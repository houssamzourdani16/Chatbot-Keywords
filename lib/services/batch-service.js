 ...
 * Optionally stores a human-readable failure_reason so the UI can show
 * exactly why the batch failed.
 */
export async function failBatch(batchId, reason = null) {
  await dbConnect();
  const update = { status: "failed", processed_at: new Date() };
