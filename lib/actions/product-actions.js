...
export async function createProduct(formData) {
  ...
  try {
    ...
    //    - If wait time is DISABLED, store 0 (instant send).
    //    - Otherwise use the Settings default / clamp to max.
    const waitingTimeEnabled = formData.get("waiting_time_enabled") !== "false";
    const rawWaitingTime = parseInt(formData.get("waiting_time"), 10);
    const waiting_time = waitingTimeEnabled
      ? await resolveWaitingTime(rawWaitingTime)
      : 0;
    ...
    const product = new Product({
      ...
      waiting_time_enabled: waitingTimeEnabled,
      waiting_time,
...
export async function updateProduct(productId, formData) {
  ...
  try {
    ...
    product.waiting_time_enabled = waitingTimeEnabled;
