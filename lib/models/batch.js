// lib/models/batch.js
import "server-only";
import mongoose from "mongoose";

const BatchSchema = new mongoose.Schema({
  // Which user owns this batch
  user_id: {
    type: mongoose.Schema.Types.ObjectId,
    ref: "User",
    required: true,
    index: true,
  },

  // Which product this batch belongs to
  product_id: {
    type: mongoose.Schema.Types.ObjectId,
    ref: "Product",
    required: true,
    index: true,
  },

  // The external sender this batch groups messages for
  sender_id: {
    type: String,
    required: true,
    index: true,
  },

  // When the debounce timer should fire (reset on each new message)
  expires_at: {
    type: Date,
    required: true,
    index: true,
  },

  // How long the debounce window is (seconds) — copied from product.waiting_time
  waiting_time: {
    type: Number,
    default: 7,
  },

  // Batch lifecycle
  status: {
    type: String,
    enum: ["open", "processing", "completed", "failed"],
    default: "open",
    index: true,
  },

  // When the batch was actually sent/processed
  processed_at: {
    type: Date,
    default: null,
  },

  // When the batch was first CLAIMED for processing. Used to detect
  // "stuck" processing batches (e.g. the serverless function died
  // mid-processing), so they can be recovered and retried.
  processing_started_at: {
    type: Date,
    default: null,
  },

  // ✅ The FULL outgoing payload that was sent to the n8n webhook
  //    (customer, product, conversation, keywords, lead, metadata,
  //    webhookUrl, executionMode). Stored after a successful send so the
  //    UI can show exactly what was delivered, even after the messages
  //    themselves are deleted.
  sent_payload: {
    type: mongoose.Schema.Types.Mixed,
    default: null,
  },

  // ✅ Why this batch failed (e.g. "Webhook returned 404",
  //    "No destination webhook URL configured"). Stored when the batch is
  //    marked failed so the UI can show the exact reason instead of a
  //    generic "failed" status.
  failure_reason: {
    type: String,
    default: null,
  },

  created_at: {
    type: Date,
    default: Date.now,
  },
});

// A product can only have ONE OPEN batch per sender at a time.
// Partial index: uniqueness is only enforced on `status: "open"`,
// so multiple completed/failed batches for the same sender are allowed.
BatchSchema.index(
  { product_id: 1, sender_id: 1 },
  { unique: true, partialFilterExpression: { status: "open" } },
);

export default mongoose.models.Batch || mongoose.model("Batch", BatchSchema);
