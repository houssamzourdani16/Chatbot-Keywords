...
const ProductSchema = new mongoose.Schema({
  ...
  // ✅ Messenger / platform access token for this product. Each page has its
  //    own access token, so the user stores it here and it is passed to the
  //    n8n webhook so the AI agent can reply on behalf of that page.
  access_token: {
    type: String,
    default: "",
  },
  // ✅ Toggle for the wait time. When FALSE the message is sent instantly
  //    (no debounce). When TRUE the batch waits `waiting_time` seconds
  //    (defaults to 7) before sending.
  ...
  // ✅ Master on/off switch for this product's webhook. When FALSE the
  //    webhook does NOT process any messages; when TRUE it acts normally.
  enabled: {
    type: Boolean,
    default: true,
  },
  // ============================================
  // ✅ WEBHOOK CALL COUNTERS
  //    Track how many times this product's webhook was called.
  //    - webhook_calls:      total calls (test + prod)
  //    - webhook_calls_test: calls to the TEST webhook
  //    - webhook_calls_prod: calls to the PRODUCTION webhook
  //    Incremented in the webhook routes on every successful call.
  // ============================================
  webhook_calls: {
    type: Number,
    default: 0,
